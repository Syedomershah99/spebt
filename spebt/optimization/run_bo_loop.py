...
def ensure_manifest_header():
    ...
    if not os.path.exists(MANIFEST_FILE):
        with open(MANIFEST_FILE, "w") as f:
            f.write("idx,config_name,aperture_diam_mm,n_apertures,work_dir,job_id,status\n")
...
def append_manifest_row(idx, config_name, diam, n_ap, work_dir, job_id, status="submitted"):
    """Append a row to the experiment manifest."""
    with open(MANIFEST_FILE, "a") as f:
        f.write(f"{idx},{config_name},{diam:.6f},{n_ap},{work_dir},{job_id},{status}\n")
...
def print_status(idx, config_name):
    ...
    try:
        ...
        best_cfg, best_ji, best_diam, best_nap = "N/A", 0.0, 0, 0
    ...
    t.add_row(
        ...
        f"{best_ji:.6e}", f"d={best_diam:.3f} n={best_nap}"
    )
    console.print(t)
...
def main():
    ...
    console.print(Panel.fit(
        "[bold green]SAI SC-SPECT BO Controller[/bold green]\n"
        "Sequential q=1 | Design: (aperture_diam, n_apertures)",
    ...
    with Progress(
        ...
        for _ in range(n_to_run):
            ...
            config_name = f"bo_{idx:04d}_ap{diam:.4f}_nap{n_ap}"
            work_dir = os.path.join(RESULTS_DIR, config_name)
            os.makedirs(work_dir, exist_ok=True)

            # 3. Append to manifest (before submission)
            append_manifest_row(idx, config_name, diam, n_ap, work_dir, "", "pending")
