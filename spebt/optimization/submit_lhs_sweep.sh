...
n_submitted=0
tail -n +2 "${MANIFEST}" | while IFS=',' read -r idx aperture_diam n_apertures work_dir; do
  config_name="lhs_${idx}_ap${aperture_diam}_nap${n_apertures}"

  echo "Submitting config ${idx}: aperture_diam=${aperture_diam} n_apertures=${n_apertures}"

  job_id=$(sbatch --parsable \
    --output="${LOG_DIR}/out/${config_name}_%j.out" \
    --error="${LOG_DIR}/err/${config_name}_%j.err" \
    --export="ALL,WORK_DIR=${work_dir},APERTURE_DIAM=${aperture_diam},N_APERTURES=${n_apertures},A_MM=${A_MM},B_MM=${B_MM},CODE_DIR=${CODE_DIR},RESULTS_CSV=${RESULTS_CSV},CONFIG_NAME=${config_name}" \
    "${SLURM_SCRIPT}")
