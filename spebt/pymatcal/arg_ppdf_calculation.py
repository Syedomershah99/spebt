...
from scanner_modeling._raytracer_2d._local_functions import (
    ppdf_2d_local,                             # does raytracing and returns ppdf values of 1 (crystal,SFOV)
    reduced_edges_2d_local,                    
    sfov_properties,                           # splits fov into sub fov
    subdivision_grid_rectangle,
)
from scanner_modeling.geometry_2d import (
    fov_tensor_dict,                           
    load_scanner_geometry_from_layout,         # extracts geometry (plates + crystals)
    load_scanner_layouts,                      # loads .tensor file
)
...
def calculate_ppdf_for_layout(layout_idx: int):
    ...
    scanner_layout_file_relative_path = (
        "/vscratch/grp-rutaoyao/Omer/spebt/geometry/scanner_layouts_24a4365260a3f68491dfa8ca55e0ecc2_rot2_ang1p0deg_trans1x1_step0p0x0p0.tensor"
    )
    ...
    mu_dict = tensor([3.5, 0.5], device=default_device)                 # materials used for raytracing
    fov_dict = fov_tensor_dict((200, 200), (10, 10), (0.0, 0.0), (3, 3))   # (pixel grid res), (fov), (centre at origin), (SFOV)
    crystal_n_subs = (3, 3)                                                # each crystal divided into 3x3 grid
    ...
    print("plate_objects_vertices shape:", plate_objects_vertices.shape)
    print("crystal_objects_vertices shape:", crystal_objects_vertices.shape)
