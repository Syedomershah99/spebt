...
# SCANNER_LAYOUTS_FILENAME = "scanner_layouts_24a4365260a3f68491dfa8ca55e0ecc2_rot1_trans1x1_step0p0x0p0.tensor"
...
ZOOM_IN_COLLIMATOR = True  # True: Zoom to collimator plates | False: View full detector ring
...
SCANNER_LAYOUTS_FILENAME = (
    ...
    "scanner_layouts_24a4365260a3f68491dfa8ca55e0ecc2_rot1_trans1x1_step0p0x0p0.tensor"
)
PPDFS_DATASET_DIR = "/vscratch/grp-rutaoyao/Omer/spebt/data/sai_10mm"
...
# --- Load Scanner Layouts ---
scanner_layouts_data, meta = load_scanner_layouts(
    SCANNER_LAYOUTS_DIR,
    SCANNER_LAYOUTS_FILENAME,
)

print("[DEBUG] scanner_layouts_data type:", type(scanner_layouts_data))
print("[DEBUG] scanner_layouts_data keys:", list(scanner_layouts_data.keys()))

# NOTE:
# load_scanner_layouts already returns a dict of layouts, e.g.:
#   {"position 000": {...}, "position 001": {...}, ...}
# So we do NOT expect a top-level "layouts" key here.
layouts_dict = scanner_layouts_data

layout_key = f"position {LAYOUT_INDEX:03d}"
if layout_key not in layouts_dict:
    raise KeyError(
        f"Layout '{layout_key}' not found. "
        f"Available layouts: {list(layouts_dict.keys())}"
    )

# Now use geometry loader (it expects the layouts dict as returned by load_scanner_layouts)
plates_vertices, detector_units_vertices = load_scanner_layout_geometries(
    LAYOUT_INDEX,
    scanner_layouts_data,   # <-- this is already the layouts dict
)

print("[DEBUG] plates_vertices shape:", tuple(plates_vertices.shape))
print("[DEBUG] detector_units_vertices shape:", tuple(detector_units_vertices.shape))

# pick the detector unit we want
detector_unit_verts = detector_units_vertices[DETECTOR_UNIT_INDEX]
detector_unit_center = detector_unit_verts.mean(dim=0)
...
# --- Load PPDF Data ---
ppdfs_hdf5_filename = f"position_{LAYOUT_INDEX:03d}_ppdfs.hdf5"
ppdf_path = os.path.join(PPDFS_DATASET_DIR, ppdfs_hdf5_filename)

print(f"[INFO] Loading PPDFs from: {ppdf_path}")
all_ppdfs = load_ppdfs_data_from_hdf5(
    PPDFS_DATASET_DIR,
    ppdfs_hdf5_filename,
    FOV_DICT,
...
ppdf_data_1d = all_ppdfs[DETECTOR_UNIT_INDEX]
ppdf_data_2d = ppdf_data_1d.view(
...
hull_points_for_sampling = cat((fov_corners, detector_unit_center.unsqueeze(0)))
sorted_hull_points = sort_points_for_hull_batch_2d(
    hull_points_for_sampling.unsqueeze(0)
).squeeze(0)
hull_2d = convex_hull_2d(sorted_hull_points)
...
# --- 3. Find Beam Boundaries ---
beam_boundaries_rads = beams_boundaries_radians(
    sampled_ppdf, sampling_rads, threshold=0.01
)

if beam_boundaries_rads.numel() == 0:
    raise ValueError(
        "No beams were found for the selected detector unit "
...
fov_points_rads = pixels_to_detector_unit_rads(
    fov_points_xy, detector_unit_center
)
...
for i in range(beams_fwhm.shape[0]):
    print(
        f"  - Beam {i+1}: FWHM = {beams_fwhm[i]:.4f} mm, "
        f"Center = {beams_weighted_centers[i].tolist()}"
    )
...
plot_polygons_from_vertices_mpl(
    ...
    ec="black",
    alpha=0.6,
    label="All Detector Units",
)

# 2. Plot FOV Boundary (Green Dashed Box)
plot_polygons_from_vertices_mpl(
    fov_corners.unsqueeze(0),
    ax=ax,
    fc="none",
    ec="green",
    lw=2,
...
# 3. Plot Sampling Arc Boundaries (Pink dashed lines)
arc_start = sampling_points[0]
arc_end = sampling_points[-1]
...
center_coords = FOV_DICT["center coordinates in mm"]
ppdf_extent = [
    (center_coords[0] - fov_size[0] / 2).item(),
    (center_coords[0] + fov_size[0] / 2).item(),
    (center_coords[1] - fov_size[1] / 2).item(),
    (center_coords[1] + fov_size[1] / 2).item(),
]
ax.imshow(
    ppdf_data_2d.T,
...
# 5. Selected Unit Highlight
plot_polygons_from_vertices_mpl(
    detector_unit_verts.unsqueeze(0),
    ax=ax,
    fc="red",
    ec="black",
    lw=1.5,
    label=f"Selected Unit ({DETECTOR_UNIT_INDEX})",
)

# 6. Lines from detector to beam centers
for i, beam_center in enumerate(beams_weighted_centers):
    line_x = [detector_unit_center[0].item(), beam_center[0].item()]
    line_y = [detector_unit_center[1].item(), beam_center[1].item()]
    ax.plot(line_x, line_y, "--", color=f"C{i}", lw=1.5)
...
intensity_scaling_factor = 40.0
direction_vectors = sampling_points - detector_unit_center
norm_vectors = direction_vectors / torch.norm(
...
ax.plot(
    visualized_curve_points[:, 0],
    visualized_curve_points[:, 1],
    "-",
    color="cyan",
    lw=2,
    label="PPDF Intensity Profile",
)
print(f"[DEBUG] ZOOM_IN_COLLIMATOR = {ZOOM_IN_COLLIMATOR}")
# 8. Zoom / Limits Logic
if ZOOM_IN_COLLIMATOR:
    if plates_vertices is not None and plates_vertices.numel() > 0:
        # Zoom to collimator plates
        all_plate_points = plates_vertices.reshape(-1, 2)
        x_min, y_min = all_plate_points.min(dim=0).values
        x_max, y_max = all_plate_points.max(dim=0).values
        ...
        ax.set_xlim(x_min - margin_x, x_max + margin_x)
        ax.set_ylim(y_min - margin_y, y_max + margin_y)
        title_suffix = " (Zoomed: collimator plates)"
    else:
        # Fallback: no plate polygons in this layout
        print("[WARN] No plate vertices found; zooming around detector + FOV instead.")
        cx, cy = detector_unit_center
        zoom_half = 40.0  # tweak as needed (mm)
        ax.set_xlim(cx - zoom_half, cx + zoom_half)
        ax.set_ylim(cy - zoom_half, cy + zoom_half)
        title_suffix = " (Zoomed: detector/FOV region)"
else:
    # Zoom to full Detector Ring + 10 mm margin
    all_det_points = detector_units_vertices.reshape(-1, 2)
    x_min, y_min = all_det_points.min(dim=0).values
    x_max, y_max = all_det_points.max(dim=0).values
    ...
    ax.set_xlim(x_min - margin, x_max + margin)
    ax.set_ylim(y_min - margin, y_max + margin)
    title_suffix = " (Full system)"

ax.set_title(f"Scanner Geometry{title_suffix} - Unit {DETECTOR_UNIT_INDEX}")
ax.set_xlabel("X (mm)")
ax.set_ylabel("Y (mm)")
ax.legend(loc="upper right")
ax.set_aspect("equal")
...
for i in range(beam_boundaries_rads.shape[0]):
    ...
    if beam_ppdf_slice.numel() > 0:
        peak_intensity = beam_ppdf_slice.max()
        peak_angle_deg = sampling_degrees[beam_mask][
            torch.argmax(beam_ppdf_slice)
        ]
        ax.text(
            peak_angle_deg,
            peak_intensity,
