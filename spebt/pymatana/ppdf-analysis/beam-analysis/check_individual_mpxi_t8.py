#!/usr/bin/env python3
"""
check_individual_mpxi_t8_like_base.py
------------------------------------
T8 version that follows the SAME logic as the base code:

- For each T8 pose (t8_00..t8_07):
...
Outputs per layout (in --out):
  1) beam_width_histogram_t8_layoutXX.png
  2) detector_beam_counts_t8_layoutXX.png
  3) fwhm_by_multiplexing_grid_t8_layoutXX.png
"""
import argparse
import os
import h5py
...
def load_masks(masks_path: str) -> torch.Tensor:  # (n_det, n_pix) e.g. (3360, 40000)
...
def run_one_layout(layout: int, data_dir: str, out_dir: str, bins: int, good_lo: float, good_hi: float):
    ...
    # ================================================================
    # Load + aggregate across T8 poses USING BASE LOGIC PER POSE
    # ================================================================
    all_fwhm = []
    all_detid = []
    all_beam_k = []

    counts_list = []  # per-pose detector multiplicity
    ...
    ax1.set_title(f"Distribution of Beam Widths (T8 aggregated, layout {layout:02d})")
    ax1.grid(axis="y", linestyle="--", alpha=0.6)

    out1 = os.path.join(out_dir, f"beam_width_histogram_t8_layout{layout:02d}.png")
    fig1.savefig(out1, dpi=300)
    plt.close(fig1)
    print(f"[SAVE] {out1}")

    # ================================================================
    # Figure 2 – beams-per-detector distribution (MAX over T8 poses)
    # ================================================================
    counts_stack = torch.stack(counts_list, dim=0)   # (8, n_det)
    counts_final = counts_stack.max(dim=0).values    # (n_det,)
    ...
    print(f"\n[LAYOUT {layout:02d}] Detectors by beam count (k), T8 aggregation = MAX over poses:")
    ...
    ax2.set_title(f"Beam Multiplicity Distribution (T8 aggregated: MAX, layout {layout:02d})")
    ax2.set_xticks(ks if max_k <= 15 else np.arange(0, max_k + 1, 2))
    ax2.grid(axis="y", linestyle="--", alpha=0.4)

    out2 = os.path.join(out_dir, f"detector_beam_counts_t8_layout{layout:02d}.png")
    ...
    fig3.suptitle(
        f"FWHM Distribution by Detector Multiplexing Degree (k) — T8 aggregated (0–0.6 mm) | layout {layout:02d}",
        fontsize=14,
    )

    out3 = os.path.join(out_dir, f"fwhm_by_multiplexing_grid_t8_layout{layout:02d}.png")
